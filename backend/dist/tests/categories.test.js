...
describe("Categories API", () => {
    describe("GET /categories", () => {
        test("should return all categories", async () => {
            const response = await (0, supertest_1.default)(app_1.default).get("/categories");
    ...
    describe("GET /categories/:id", () => {
        test("should return category by id", async () => {
            const categories = await (0, supertest_1.default)(app_1.default).get("/categories");
            const categoryId = categories.body[0].id;
            const response = await (0, supertest_1.default)(app_1.default).get(`/categories/${categoryId}`);
            expect(response.status).toBe(200);
        ...
        test("should return 404 for missing category", async () => {
            const response = await (0, supertest_1.default)(app_1.default).get("/categories/999999");
    ...
    describe("POST /categories", () => {
        test("should create a category", async () => {
            const response = await (0, supertest_1.default)(app_1.default).post("/categories").send({
    ...
    describe("PUT /categories/:id", () => {
        test("should update a category", async () => {
            const created = await (0, supertest_1.default)(app_1.default).post("/categories").send({
    ...
    describe("DELETE /categories/:id", () => {
        test("should delete a category", async () => {
            ...
            expect(created.status).toBe(201);
            const response = await (0, supertest_1.default)(app_1.default).delete(`/categories/${created.body.id}`);
            expect(response.status).toBe(200);
