...
describe("Comments API", () => {
    describe("GET /comments/event/:eventId", () => {
        test("should return comments for an existing event", async () => {
            const response = await (0, supertest_1.default)(app_1.default).get("/comments/event/1");
        ...
        test("should return 404 for comments of a missing event", async () => {
            const response = await (0, supertest_1.default)(app_1.default).get("/comments/event/999999");
    ...
    describe("POST /comments", () => {
        test("should create a comment with valid data when user is authenticated", async () => {
            ...
            expect(response.body).toHaveProperty("message", "This is a test comment.");
        });
        test("should return 401 when creating comment without token", async () => {
            const response = await (0, supertest_1.default)(app_1.default).post("/comments").send({
                eventId: 1,
                author: "Test Author",
    ...
    describe("DELETE /comments/:commentId", () => {
        ...
        test("should return 401 when deleting comment without token", async () => {
            const response = await (0, supertest_1.default)(app_1.default).delete("/comments/1");
            expect(response.status).toBe(401);
            expect(response.body).toHaveProperty("message", "Authorization token is required.");
        });
